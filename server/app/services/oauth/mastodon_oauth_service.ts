...
import { BaseOAuthService, OAuthProfile, OAuthTokens } from './base_oauth_service.js'
...
export class MastodonOAuthService extends BaseOAuthService {
  ...
  constructor() {
    ...
    this.redirectUri = `${env.get('APP_URL')}/api/v1/users/oauth/mastodon/callback`
  }

  private assertConfigured() {
    if (!this.clientId || !this.clientSecret) {
      throw new Error('Mastodon OAuth is not configured (MASTODON_CLIENT_ID / MASTODON_CLIENT_SECRET missing)')
    }
  }
  ...
  async exchangeCode(code: string): Promise<OAuthTokens> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Mastodon token exchange failed (${res.status}): ${text}`)
    }
  ...
  async getProfile(accessToken: string): Promise<OAuthProfile> {
    const res = await fetch(`https://${this.instance}/api/v1/accounts/verify_credentials`, {
